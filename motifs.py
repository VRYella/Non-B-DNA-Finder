...
def find_gquadruplex(seq):
    ...
    return [
        dict(Class="Quadruplex", Subtype="Canonical_G-Quadruplex", Start=m.start()+1, End=m.start()+len(m.group(1)), Length=len(m.group(1)),
...
def find_relaxed_gquadruplex(seq):
    pattern = r"(?=(G{3,}[ATGC]{1,12}G{3,}[ATGC]{1,12}G{3,}[ATGC]{1,12}G{3,}))"
    return [
        dict(Class="Quadruplex", Subtype="Relaxed_G-Quadruplex", Start=m.start()+1, End=m.start()+len(m.group(1)), Length=len(m.group(1)),
             Sequence=wrap(m.group(1)), ScoreMethod="G4Hunter", Score=f"{g4hunter_score(m.group(1)):.2f}")
        for m in overlapping_finditer(pattern, seq)
    ]

def find_bulged_gquadruplex(seq):
    # Simple version: Allow up to 3 non-Gs in G runs
...
def find_imotif(seq):
    ...
    return [
        dict(Class="Quadruplex", Subtype="i-Motif", Start=m.start()+1, End=m.start()+len(m.group(1)), Length=len(m.group(1)),
             Sequence=wrap(m.group(1)), ScoreMethod="G4Hunter", Score=f"{-g4hunter_score(m.group(1).replace('C','G')):.2f}")
        for m in overlapping_finditer(pattern, seq)
    ]
...
def find_gtriplex(seq):
    ...
    return [
        dict(Class="Triplex", Subtype="G-Triplex", Start=m.start()+1, End=m.start()+len(m.group(1)), Length=len(m.group(1)),
             Sequence=wrap(m.group(1)), ScoreMethod="G4Hunter", Score=f"{g4hunter_score(m.group(1)):.2f}")
        for m in overlapping_finditer(pattern, seq)
    ]

def find_bipartite_gquadruplex(seq):
    pattern = r"(?=(G{3,}[ATGC]{1,30}G{3,}[ATGC]{1,30}G{3,}[ATGC]{1,30}G{3,}))"
    return [
        dict(Class="Quadruplex", Subtype="Bipartite_G-Quadruplex", Start=m.start()+1, End=m.start()+len(m.group(1)), Length=len(m.group(1)),
             Sequence=wrap(m.group(1)), ScoreMethod="G4Hunter", Score=f"{g4hunter_score(m.group(1)):.2f}")
        for m in overlapping_finditer(pattern, seq)
    ]

def find_multimeric_gquadruplex(seq):
    pattern = r"(?=((G{3,}[ATGC]{1,12}){4,}))"
    return [
...
def find_zdna(seq):
    pattern = r"(?=(?:CG){6,})"
    return [
        dict(Class="Z-DNA", Subtype="CG_Repeat", Start=m.start()+1, End=m.start()+len(m.group(0)), Length=len(m.group(0)),
             Sequence=wrap(m.group(0)), ScoreMethod="ZSeeker", Score=f"{zseeker_score(m.group(0)):.2f}")
        for m in overlapping_finditer(pattern, seq)
    ]
...
def find_quadruplex_triplex_hybrid(seq):
    # Example: Overlapping G4 and triplex
    g4_pattern = r"(?=(G{3,}[ATGC]{1,7}G{3,}[ATGC]{1,7}G{3,}[ATGC]{1,7}G{3,}))"
    triplex_pattern = r"(?=(G{3,}[ATGC]{1,7}G{3,}[ATGC]{1,7}G{3,}))"
    hits = []
...
def find_cruciform_triplex_junction(seq):
    # Example: Overlapping cruciform and triplex
...
def find_g4_imotif_hybrid(seq):
    g4_pattern = r"(?=(G{3,}[ATGC]{1,7}G{3,}[ATGC]{1,7}G{3,}[ATGC]{1,7}G{3,}))"
    imotif_pattern = r"(?=(C{3,}[ATGC]{1,7}C{3,}[ATGC]{1,7}C{3,}[ATGC]{1,7}C{3,}))"
    hits = []
    g4_hits = [(m.start(), m.end()) for m in overlapping_finditer(g4_pattern, seq)]
...
def all_motifs(seq):
    # Call all motif finders and collect results
    motif_funcs = [
        ...
        find_quadruplex_triplex_hybrid, find_cruciform_triplex_junction, find_g4_imotif_hybrid, find_local_bent
    ]
    all_hits = []
...
def find_hotspots(seq, motif_hits, window=100, min_count=3):
    # motif_hits: list of dicts, each with Start and End (1-based)
    n = len(seq)
    hotspots = []
    motif_positions = []
    for hit in motif_hits:
        motif_positions.append((hit["Start"], hit["End"]))
    for i in range(1, n-window+2):
        count = 0
        region_start = i
        region_end = i + window - 1
        for mstart, mend in motif_positions:
            if mstart <= region_end and mend >= region_start:
                count += 1
        if count >= min_count:
            hotspots.append(dict(RegionStart=region_start, RegionEnd=region_end, MotifCount=count))
    # Remove duplicates (regions with same start)
